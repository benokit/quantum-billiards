...
def RPW_sym(i,k,x,y, dphi = 0.1, sym_x = "odd", sym_y = "odd"):
    ...
    if sym_x == "odd" and sym_y == "odd":
        return np.sin(k*vx*x)* np.sin(k*vy*y)
        
    if sym_x == "odd" and sym_y == "even":
        return np.cos(k*vx*x)* np.sin(k*vy*y)

    if sym_x == "even" and sym_y == "odd":
        return np.sin(k*vx*x)* np.cos(k*vy*y)
        
    if sym_x == "even" and sym_y == "even":
        return np.cos(k*vx*x)* np.cos(k*vy*y)
...
def RPW_sym_grad(i,k,x,y, dphi = 0.1, sym_x = "odd", sym_y = "odd"):
    ...
    if sym_x == "odd" and sym_y == "odd":
        dx = k*vx* np.cos(k*vx*x)* np.sin(k*vy*y)
        dy = k*vy* np.sin(k*vx*x)* np.cos(k*vy*y)
        return dx, dy
        
    if sym_x == "odd" and sym_y == "even":
        dx = -k*vx* np.sin(k*vx*x)* np.sin(k*vy*y)
        dy = k*vy* np.cos(k*vx*x)* np.cos(k*vy*y)
        return dx, dy

    if sym_x == "even" and sym_y == "odd":
        dx = k*vx* np.cos(k*vx*x)* np.cos(k*vy*y)
        dy = -k*vy* np.sin(k*vx*x)* np.sin(k*vy*y)
        return dx, dy
        
    if sym_x == "even" and sym_y == "even":
        dx = -k*vx* np.sin(k*vx*x)* np.cos(k*vy*y)
        dy = -k*vy* np.cos(k*vx*x)* np.sin(k*vy*y)
        return dx, dy
...
def RPW_sym_dk(i,k,x,y, dphi = 0.1, sym_x = "odd", sym_y = "odd"):
    ...
    if sym_x == "odd" and sym_y == "odd":
        return vx*x*np.cos(k*vx*x)*np.sin(k*vy*y) + vy*y*np.sin(k*vx*x)*np.cos(k*vy*y)
     
    if sym_x == "odd" and sym_y == "even":
        return -vx*x*np.sin(k*vx*x)*np.sin(k*vy*y) + vy*y*np.cos(k*vx*x)*np.cos(k*vy*y)

    if sym_x == "even" and sym_y == "odd":
        return vx*x*np.cos(k*vx*x)*np.cos(k*vy*y) + vy*y*np.sin(k*vx*x)*np.sin(k*vy*y)

    if sym_x == "even" and sym_y == "even":
        return -vx*x*np.sin(k*vx*x)*np.cos(k*vy*y) - vy*y*np.cos(k*vx*x)*np.sin(k*vy*y)
